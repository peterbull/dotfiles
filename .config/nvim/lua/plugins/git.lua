...
local function cleanup_iadd()
	...
	local out = vim.fn.system(
		vim.list_extend(
			{ "git", "-C", pending.toplevel, "-c", "core.quotePath=false", "ls-files", "--stage", "--" },
			pending.files
		)
	)
	local still = {}
		...
		keys = {
			{
				...
				function()
					...
					-- Everything since the previous commit on this branch: the last
					-- commit's changes plus uncommitted work, untracked included.
					local prev = vim.fn.system({ "git", "rev-parse", "--verify", "HEAD~1" }):gsub("%s", "")
					if prev == "" or vim.v.shell_error ~= 0 then
						-- No parent (root commit or detached HEAD): fall back to the
						-- plain worktree vs HEAD view (untracked shown natively).
						vim.cmd("DiffviewOpen --untracked-files=all")
						return
					end
					open_branch_diff(prev)
				end,
				desc = "Diff since previous commit on this branch, incl. untracked",
			},
			{
				"<leader>gm",
				function()
					...
					if merge_base == "" or vim.v.shell_error ~= 0 then
						...
						merge_base =
							vim.fn.system({ "git", "merge-base", "HEAD", "origin/" .. default_branch }):gsub("%s", "")
					end
					if merge_base == "" or vim.v.shell_error ~= 0 then
